...
function ProductCard() {
  ...
  const { mode, product, searchkey, filterType,
    filterPrice, addCartFirebase,addCartLoading,setAddCartLoading } = context
...

       {
        ...
        {product.filter((obj) => obj.title.toLowerCase().includes(searchkey))
          ...
          .filter((obj) => obj.price.includes(filterPrice)).slice(0, 8).map((item, index) => {
            ...
            // const isItemLoading = addCartLoading[id];
            return (
              <div key={index} className="relative m-10 w-full snap-start max-w-xs overflow-hidden rounded-lg bg-white shadow-md">
                ...
                <div className="mt-4 px-5 pb-5" style={{ background: mode === 'dark' ? '' : '' }}>
                  ...
                  <div key={index} className="flex items-center justify-between">
                    ...
                    <a
                        key={index}
                        className="flex items-center cursor-pointer rounded-md bg-pink-600 px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-pink-500 focus:outline-none focus:ring-4 focus:ring-blue-300"
                        onClick={() => {
                          // setAddCartLoading((prevLoading) => ({ ...prevLoading, [id]: true }));
                          addCart(item)
                        }}
                      >
